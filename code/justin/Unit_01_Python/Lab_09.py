# Full Stack Bootcamp - Unit 01 Lab 09
...
'''
Compute the ARI for a given body of text loaded in from a file. The automated readability index (ARI) is a formula for computing
...
The score is computed by multiplying the number of characters divided by the number of words by 4.71, adding the number of words
divided by the number of sentences multiplied by 0.5, and subtracting 21.43. If the result is a decimal, always round up. Scores
greater than 14 should be presented as having the same age and grade level as scores of 14.
...
def compute_ARI(character_count, word_count, sentence_count):
    raw_ARI = 4.71 * (character_count / word_count) + 0.5 * (word_count / sentence_count) - 21.43
    
    # Round up to nearest integer
    rounded_ARI = math.ceil(raw_ARI)

    # Clamp between 1 and 14
    rounded_ARI = 1 if rounded_ARI < 1 else 14 if rounded_ARI > 14 else rounded_ARI

    return rounded_ARI

def test_compute_ARI():
    assert compute_ARI(623, 151, 6) == 11
    assert compute_ARI(-623, 151, 6) == 1
    assert compute_ARI(9623, 151, 6) == 14

def get_grade_level(ARI):
    grade_level = []
    try:
        grade_level = grade_level_ARI_map[ARI]
    except KeyError as exception:
        print(exception.args)
        grade_level = ['KeyError', f"{exception.args[0]} is an invalid key"]
    finally:
        return grade_level

def test_get_grade_level():
    assert get_grade_level(0) == ['KeyError', '0 is an invalid key']
    assert get_grade_level(15) == ['KeyError', '15 is an invalid key']

    assert get_grade_level(1) == ['5-6', 'Kindergarten']
    assert get_grade_level(2) == ['6-7', 'First Grade']
    assert get_grade_level(3) == ['7-8', 'Second Grade']
    assert get_grade_level(4) == ['8-9', 'Third Grade']
    assert get_grade_level(5) == ['9-10', 'Fourth Grade']
    assert get_grade_level(6) == ['10-11', 'Fifth Grade']
    assert get_grade_level(7) == ['11-12', 'Sixth Grade']
    assert get_grade_level(8) == ['12-13', 'Seventh Grade']
    assert get_grade_level(9) == ['13-14', 'Eighth Grade']
    assert get_grade_level(10) == ['14-15', 'Ninth Grade']
    assert get_grade_level(11) == ['15-16', 'Tenth Grade']
    assert get_grade_level(12) == ['16-17', 'Eleventh Grade']
    assert get_grade_level(13) == ['17-18', 'Twelfth Grade']
    assert get_grade_level(14) == ['18-22', 'College']

def get_ARI_data_from_text(text):
    ARI_data = {
        'characters': 0,
        'words': 0,
        'sentences': 0
    ...
    ARI_data['sentences'] = len(sentence_list)
    ...
    ARI_data['words'] = words
    ARI_data['characters'] = characters


    return ARI_data

def test_get_ARI_data_from_text():
    sample_text = 'The rule of rhythm in prose is not so intricate. Here, too, we write in groups, or phrases, as I prefer to call them, for the prose phrase is greatly longer and is much more nonchalantly uttered than the group in verse; so that not only is there a greater interval of continuous sound between the pauses, but, for that very reason, word is linked more readily to word by a more summary enunciation. Still, the phrase is the strict analogue of the group, and successive phrases, like successive groups, must differ openly in length and rhythm. The rule of scansion in verse is to suggest no measure but the one in hand; in prose, to suggest no measure at all. Prose must be rhythmical, and it may be as much so as you will; but it must not be metrical. It may be anything, but it must not be verse.'
    assert get_ARI_data_from_text(sample_text) == { 'characters': 623, 'words': 151, 'sentences': 6 }

def build_ARI_message(file_name, ARI):
    grade_level = get_grade_level(ARI)

    if grade_level[0] == 'KeyError':
        return None
    else:
        line1 = f'The ARI for {file_name} is {ARI}\n'
        line2 = f'This corresponds to a{" " + grade_level[1] if ARI != 12 else "n " + grade_level[1]} level of difficulty\n'
        line3 = f'that is suitable for an average person {grade_level[0]} years old.'
        return line1 + line2 + line3

def test_build_ARI_message():
    assert build_ARI_message('sample.txt', 1) == 'The ARI for sample.txt is 1\nThis corresponds to a Kindergarten level of difficulty\nthat is suitable for an average person 5-6 years old.'
    assert build_ARI_message('sample.txt', 2) == 'The ARI for sample.txt is 2\nThis corresponds to a First Grade level of difficulty\nthat is suitable for an average person 6-7 years old.'
    assert build_ARI_message('sample.txt', 3) == 'The ARI for sample.txt is 3\nThis corresponds to a Second Grade level of difficulty\nthat is suitable for an average person 7-8 years old.'
    assert build_ARI_message('sample.txt', 4) == 'The ARI for sample.txt is 4\nThis corresponds to a Third Grade level of difficulty\nthat is suitable for an average person 8-9 years old.'
    assert build_ARI_message('sample.txt', 5) == 'The ARI for sample.txt is 5\nThis corresponds to a Fourth Grade level of difficulty\nthat is suitable for an average person 9-10 years old.'
    assert build_ARI_message('sample.txt', 6) == 'The ARI for sample.txt is 6\nThis corresponds to a Fifth Grade level of difficulty\nthat is suitable for an average person 10-11 years old.'
    assert build_ARI_message('sample.txt', 7) == 'The ARI for sample.txt is 7\nThis corresponds to a Sixth Grade level of difficulty\nthat is suitable for an average person 11-12 years old.'
    assert build_ARI_message('sample.txt', 8) == 'The ARI for sample.txt is 8\nThis corresponds to a Seventh Grade level of difficulty\nthat is suitable for an average person 12-13 years old.'
    assert build_ARI_message('sample.txt', 9) == 'The ARI for sample.txt is 9\nThis corresponds to a Eighth Grade level of difficulty\nthat is suitable for an average person 13-14 years old.'
    assert build_ARI_message('sample.txt', 10) == 'The ARI for sample.txt is 10\nThis corresponds to a Ninth Grade level of difficulty\nthat is suitable for an average person 14-15 years old.'
    assert build_ARI_message('sample.txt', 11) == 'The ARI for sample.txt is 11\nThis corresponds to a Tenth Grade level of difficulty\nthat is suitable for an average person 15-16 years old.'
    assert build_ARI_message('sample.txt', 12) == 'The ARI for sample.txt is 12\nThis corresponds to an Eleventh Grade level of difficulty\nthat is suitable for an average person 16-17 years old.'
    assert build_ARI_message('sample.txt', 13) == 'The ARI for sample.txt is 13\nThis corresponds to a Twelfth Grade level of difficulty\nthat is suitable for an average person 17-18 years old.'
    assert build_ARI_message('sample.txt', 14) == 'The ARI for sample.txt is 14\nThis corresponds to a College level of difficulty\nthat is suitable for an average person 18-22 years old.'
    
    assert build_ARI_message('sample.txt', 0) == None
    assert build_ARI_message('sample.txt', 19) == None
...
def main():
    ...
    ARI_data = get_ARI_data_from_text(text)
    ARI = compute_ARI(ARI_data['characters'], ARI_data['words'], ARI_data['sentences'])

    message = build_ARI_message(file_name, ARI)
    print(f"\n{message}\n")
