...
SHOULD_PRINT_STUFF_TO_CONSOLE = True
SHOULD_PRINT_STUFF_TO_CONSOLE = False


# TODO: Learn a better way to choose rendered "template" than routing to two separate templates.
# TODO: Learn how to pass the 'pk' from 'add()' view to the appropriate 'index' view. Might need to pass it through the url. Would need to add <int:pk> to path().
def index(request, pk=''):
    """
    Renders template 'index.html' to display the current shortened URLs and allows user to create a new one.
    """
    utils.print_meta_data_to_console_return_meta_data_dictionary(request, SHOULD_PRINT_STUFF_TO_CONSOLE)
    ...
    #### Pass the pk functionality ####
    if pk != '':
        print(f"pk of newly created ShortCode: {pk}")
        newest_url_set = get_object_or_404(ShortCode, pk=pk)
        # Add 'newest_url_set' to context for display on page.
        context['newest_url_set'] = newest_url_set
    ###################################

    return render(request, 'short/index.html', context)


def index_cards(request, pk=''):
    """
...
def add(request):
    ...
    """
    utils.print_post_data_to_console_return_post_data_dictionary(request, SHOULD_PRINT_STUFF_TO_CONSOLE)
    utils.print_meta_data_to_console_return_meta_data_dictionary(request, SHOULD_PRINT_STUFF_TO_CONSOLE)
    ...
    print(f"Where we're from: {where_we_came_from}")
    ...
    print(f"Where we're going: {where_to_return_to_dict[where_we_came_from]}")
    return HttpResponseRedirect(reverse(where_to_return_to_dict[where_we_came_from]) + str(new_code_pk))
