...
function convert(number) {
    ...
    if (number < 100) {
        if (number in ones_dict) {
            ...
            return`${tens_dict[tens_digit2]}-${ones_dict[ones_digit]}`
        }
    }
    else{
        if (number in hunds_dict) {
            return hunds_dict[number]
        }
        else if (tens_digit1 == 0){
            return`${hunds_dict[hunds_digit2]}-${ones_dict[ones_digit]}`
        }
        else if (tens_digit1 == 1){
            return`${hunds_dict[hunds_digit2]}-${teens_dict[teens_digit]}`
        }
        else if (ones_digit == 0){
            return`${hunds_dict[hunds_digit2]}-${tens_dict[tens_digit2]}`
        }         
        else if (hunds_digit2 in hunds_dict && tens_digit2 in tens_dict && ones_digit in ones_dict){
            return`${hunds_dict[hunds_digit2]}-${tens_dict[tens_digit2]}-${ones_dict[ones_digit]}`
        }
    }
}
